...
delete_ssh_user() {
    ...
    if gum confirm "Are you sure you want to delete ${USERNAME}?"; then
        userdel -r "$USERNAME" &>/dev/null
        if [ $? -eq 0 ]; then
            log_success "SSH user ${USERNAME} deleted successfully."
        else
            log_error "Failed to delete SSH user ${USERNAME}."
        fi
    else
